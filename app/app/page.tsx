...
/*
 * /app — the standalone Sycamore Square OS mockup (full-screen). This is the
 * interactive product the proposal embeds in its "Live demo" section via an
 * iframe, and also what "Open full screen" links to. Self-contained: sidebar
 * OS shell + Internal/Client-portal toggle.
 */
...
import { SycBid } from "@/components/sycbid";
...
const VIEWS: AppView[] = ["home", "syckpi", "sycflow", "sycbid", "sycai"];
...
function PageInner() {
  ...
  useEffect(() => {
    ...
    if (changed) router.replace(`/app?${params.toString()}`, { scroll: false });
  }, [view, env, router]);
